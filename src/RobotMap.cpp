...
namespace RobotMap
{
	namespace Controls
	{
		...
		const float toggleCollectWheelSolenoid = 1;
		const float toggleGrabSolenoid = 2;
		const float dumpToteGroup = 4;
		const float dumpTote = 3;
		const float prepareHumanGroup = 6;
		const float prepareHuman = 5;
		const float rackStackGroup = 7;
